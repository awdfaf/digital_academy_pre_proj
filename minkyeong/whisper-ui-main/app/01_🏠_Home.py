...
#     high_freq = 1000
#     # wav 파일 로드
#     data/media/news/audio.wav
#     rate, data = 0audio(media["filepath"])
...
# #     # 필터 적용
#     filtered_data = lfilter(b, a, source)
...
with st.sidebar.expander("➕ &nbsp; Add Media", expanded=False):
    ...
    if add_media:
        ...
        if source_type == "YouTube":
            ...
            if input_files:
                source = input_files
                st.write(source, input_files)
                
# [
# 0:"UploadedFile(id=9, name='0005.wav', type='audio/wav', size=116238)"
# ]
       ...
       ### "UploadedFile(id=2, name='news.wav', type='audio/wav', size=1729834)"
...
if st.session_state.list_mode:
    ...
    with file_tab:
        ...
        for media in media_objs:
            ...
            ### st.write(media, meta_col, media_col)
