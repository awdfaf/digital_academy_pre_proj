...
def classifyPose(landmarks, output_image, display=False):
    ...
    if right_elbow_angle < 120:
        label = 'right elbow fold'

        if left_elbow_angle < 120:
            label = 'left elbow fold'
...
if __name__ == '__main__':
    ...
    process_yolov7 = multiprocessing.Process(target = yolov7)
    process_renewal = multiprocessing.Process(target = renewal)
    process_mediapipe = multiprocessing.Process(target = mediapipe)
    
    
    
    process_renewal.start()
    process_mediapipe.start()
    sleep(3)
    process_yolov7.start()
    
    process_yolov7.join()
    process_renewal.join()   
    process_mediapipe.join()
